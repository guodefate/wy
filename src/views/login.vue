...
      const verification =this.ReginForm.username;
       const url = " "
       console.log("url",url);
...
   const phoneData =this.ReginForm.username;
   const mCodeData=this.verification;
  //  手机注册
//emulateJSON:true设置后post可跨域
  const url = "http://192.168.1.123:8081/api/user/register"
      this.$http.post(url,{username,password},{emulateJSON:true}).then(function (response) 
 {
     //登录后跳转的页面
        this.$router.push('/');
      }, function (error) {
